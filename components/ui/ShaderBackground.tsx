...
const ShaderBackground: React.FC<ShaderBackgroundProps> = ({ className }) => {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      // Set canvas resolution to match its display size (responsive)
    ...
    const render = () => {
      ...
      gl.clearColor(0.0, 0.0, 0.0, 0.0); // Transparent background to allow blending if needed
      gl.clear(gl.COLOR_BUFFER_BIT);

      gl.useProgram(programInfo.program);

      // Guard against null locations if shader failed compilation partially or optimizations removed uniforms
